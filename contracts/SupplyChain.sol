...
contract SupplyChain {
  ...
  constructor() public {
    ...
    owner = msg.sender;
    // 2. Initialize the sku count to 0. Question, is this necessary? No not really, intial value of uint is 0 
    skuCount = 0;
  }
  ...
  function buyItem(uint sku) public payable forSale(sku) paidEnough(items[sku].price) checkValue(sku) {
    Item storage temp = items[sku];
    temp.seller.transfer(temp.price);
    temp.buyer = msg.sender;
    temp.state = State.Sold;
    emit LogSold(sku);
  }
